...
impl SshService {
    ...
    pub async fn connect(&self, target: &str, user: Option<String>, port: Option<u16>, 
                        kerberos: Option<bool>, bastion: Option<String>, no_bastion: bool, bastion_user: Option<String>, 
                        key: Option<String>) -> Result<()> {
        ...
        info!("Connection '{}' not found, attempting direct connection", target);
    ...
    async fn execute_ssh(&self, connection: &Connection) -> Result<()> {
        ...
        if let Some(bastion) = &connection.bastion {
            ...
            info!("Connecting via bastion {} to target {}", bastion, connection.host);
        } else {
            // Direct connection
    ...
    pub async fn add_connection(&self, name: String, host: String, user: Option<String>, 
                               port: Option<u16>, kerberos: Option<bool>, bastion: Option<String>, 
                               no_bastion: bool, bastion_user: Option<String>, key: Option<String>, tags: Vec<String>) -> Result<()> {
    ...
    pub async fn list_connections(&self, tag_filter: Option<&str>, recent_only: bool) -> Result<Vec<Connection>> {
    ...
    async fn ensure_kerberos_ticket(&self) -> Result<()> {
        ...
        let ticket_status = TokioCommand::new("klist")
            .arg("-s")
            .output()
            .await?;
        ...
        if kinit_status.status.success() {
            ...
            Err(anyhow::anyhow!("Failed to create Kerberos ticket: {}", error_output))
        }
    }
}

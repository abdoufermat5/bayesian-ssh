...
pub async fn execute(
    ...
    info!("Listing connections (tag: {:?}, recent: {}, detailed: {})", tag, recent, detailed);
    ...
    for (i, conn) in connections.iter().enumerate() {
        if detailed {
            ...
            println!("   Kerberos: {}", if conn.use_kerberos { "✅" } else { "❌" });
            ...
            println!("{}. {}{} {}{}{}", 
                ...
                kerberos_icon,
                last_used_info,
                if !conn.tags.is_empty() { format!(" [{}]", conn.tags.join(", ")) } else { String::new() }
            );
        }
    }

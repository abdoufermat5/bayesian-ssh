...
pub async fn execute(file: String, force: bool, config: AppConfig) -> Result<()> {
    ...
    info!("Restoring database from {:?} to {:?}", restore_path, default_db_path);
    
    // Create a safety backup before overwriting
    if default_db_path.exists() {
        let parent = default_db_path.parent().unwrap_or_else(|| std::path::Path::new(""));
    ...
    println!("✅ Database successfully restored from {}", restore_path.display());

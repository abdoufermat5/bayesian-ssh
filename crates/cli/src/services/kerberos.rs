...
/// Snapshot of the local Kerberos client state, serialized for the GUI.
...
/// Probe the local Kerberos client and report ticket/tooling state.
pub fn get_status() -> Result<KerberosStatus, String> {
...
/// Renew the current Kerberos ticket, falling back to a forwardable re-acquire
/// when `password` is provided and renewal fails.
pub fn renew_ticket(password: Option<String>) -> Result<KerberosStatus, String> {
    if !command_exists("kinit") {
...
/// Acquire a fresh Kerberos ticket with the given options.
